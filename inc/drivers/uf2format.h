...
static inline bool is_uf2_block(void* data)
{
    ...
    return bl->magicStart0 == UF2_MAGIC_START0 && bl->magicStart1 == UF2_MAGIC_START1 && bl->magicEnd == UF2_MAGIC_END;
}
...
{
    if (in_uf2_bootloader_space(UF2_INFO_ADDR)) return UF2_INFO_ADDR;
    return "N/A";
}
...
static inline void hf2_handover(uint8_t ep)
{
    ...
    if (in_uf2_bootloader_space(board_info) && in_uf2_bootloader_space((const void*)fn) && ((uint32_t)fn & 1)) {
        // Pass control to bootloader; never returns
        fn(ep & 0xf);
    }
}

static inline void check_uf2_handover(uint8_t* buffer, uint32_t blocks_remaining, uint8_t ep_in, uint8_t ep_out,
                                      uint32_t cbw_tag)
{
    if (!is_uf2_block(buffer)) return;
