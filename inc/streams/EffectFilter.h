...
class EffectFilter : public DataSource, public DataSink {
  ...
  public:
     ...
     *
     * @param deepCopy Set to true to copy incoming data into a freshly allocated buffer, or false to change data in
     * place.
     */
    void setDeepCopy(bool deepCopy);

    /**
     * Default effect - a simple pass through filter. Override this method in subclasses to create specialist
     * effects/filters.
     *
     * @param inputBuffer the buffer containing data to process.
     * @param outputBuffer the buffer in which to store the filtered data. n.b. MAY be the same memory as the input
     * buffer.
