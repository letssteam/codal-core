...
{
    if (!(status & CODAL_SERIAL_STATUS_RX_BUFF_INIT)) return;
    ...
    while (delimeterOffset < delimLength) {
        ...
        if (this->delimeters.charAt(delimeterOffset) == c) Event(this->id, CODAL_SERIAL_EVT_DELIM_MATCH);
...
{
    if (!(status & CODAL_SERIAL_STATUS_TX_BUFF_INIT)) return;
...
int Serial::setTxInterrupt(uint8_t* string, int len, SerialMode mode)
{
    ...
    while (copiedBytes < len) {
        ...
        if (nextHead == txBuffTail) {
            ...
            if (mode == SYNC_SLEEP) fiber_wait_for_event(DEVICE_ID_NOTIFY, CODAL_SERIAL_EVT_TX_EMPTY);
...
int Serial::initialiseRx()
{
    ...
    if ((this->rxBuff = (uint8_t*)malloc(rxBuffSize)) == NULL) return DEVICE_NO_RESOURCES;
...
int Serial::initialiseTx()
{
    ...
    if ((this->txBuff = (uint8_t*)malloc(txBuffSize)) == NULL) return DEVICE_NO_RESOURCES;
 ...
 */
void Serial::circularCopy(uint8_t* circularBuff, uint8_t circularBuffSize, uint8_t* linearBuff, uint16_t tailPosition,
...
int Serial::send(uint8_t* buffer, int bufferLen, SerialMode mode)
{
    ...
    if (bufferLen <= 0 || buffer == NULL) return DEVICE_INVALID_PARAMETER;
...
{
    if (mode == ASYNC) {
        if (!isReadable()) return DEVICE_NO_DATA;
    }
    ...
    if (mode == SYNC_SLEEP) {
        if (!isReadable()) eventAfter(1, mode);
    }
...
ManagedString Serial::read(int size, SerialMode mode)
{
    ...
    if (returnedSize <= 0) return ManagedString();
...
int Serial::read(uint8_t* buffer, int bufferLen, SerialMode mode)
{
    ...
    if (mode == SYNC_SLEEP) {
        if (bufferLen > rxBufferedSize()) eventAfter(bufferLen - rxBufferedSize(), mode);
...
{
    if (rxInUse()) return ManagedString();
    ...
    while (localTail != rxBuffHead && foundIndex == -1) {
        ...
        for (int delimeterIterator = 0; delimeterIterator < delimeters.length(); delimeterIterator++)
            if (delimeters.charAt(delimeterIterator) == c) foundIndex = localTail;
    ...
    if (mode == SYNC_SLEEP && foundIndex == -1) {
        ...
        if (foundIndex < 0) foundIndex += rxBuffSize;
    ...
    if (foundIndex >= 0) {
        ...
        int localBuffSize =
            (preservedTail > foundIndex) ? (rxBuffSize - preservedTail) + foundIndex : foundIndex - preservedTail;
...
{
    if (baudrate < 0) return DEVICE_INVALID_PARAMETER;

    int ret = setBaudrate((uint32_t)baudrate);

    if (ret == DEVICE_OK) this->baudrate = baudrate;
...
int Serial::redirect(Pin& tx, Pin& rx)
{
    ...
    if (txBufferedSize() > 0) disableInterrupt(TxInterrupt);
    ...
    if (txBufferedSize() > 0) enableInterrupt(TxInterrupt);
...
int Serial::eventOn(ManagedString delimeters, SerialMode mode)
{
    ...
    // block!
    if (mode == SYNC_SLEEP) fiber_wait_for_event(this->id, CODAL_SERIAL_EVT_DELIM_MATCH);
...
{
    if (rxBuffTail > rxBuffHead) return (rxBuffSize - rxBuffTail) + rxBuffHead;
...
{
    if (txBuffTail > txBuffHead) return (txBuffSize - txBuffTail) + txBuffHead;
