...
{
    uint8_t sum = 0;
    for (int i = 0; i < 11; ++i) sum = ((sum & 1) << 7) + (sum >> 1) + *name++;
    return sum;
}
...
{
    if (!c) return 0;
    return ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z') || ('0' <= c && c <= '9') || strchr("_-", c);
}
...
void GhostFAT::buildBlock(uint32_t block_no, uint8_t* data)
{
    ...
    if (block_no == 0) {
        ...
        if (sectionIdx >= SECTORS_PER_FAT) sectionIdx -= SECTORS_PER_FAT;
        ...
        for (GFATEntry* p = files; p; p = p->next) {
            ...
            for (int i = 0; i < n; i++) WRITE_ENT(cl + 1);
            WRITE_ENT(0xffff);
        }
    }
...
{
    if (files == NULL || filesFinalized()) return;
...
GFATEntry* GhostFAT::addFile(GFATReadCallback read, void* userdata, const char* filename, uint32_t size, uint8_t dirid)
{
    if (filesFinalized()) target_panic(DEVICE_USB_ERROR);
