...
{
    // Are we playing back?
    if (this->state != REC_STATE_PLAYING) return ManagedBuffer();
    ...
    if (downStream != NULL) downStream->pullRequest();
...
{
    // Are we recording?
    if (this->state != REC_STATE_RECORDING) return DEVICE_BUSY;
    ...
    if (!isFull()) {
        ...
        if (block == NULL) return DEVICE_NO_RESOURCES;
        block->next = NULL;
...
bool StreamRecording::recordAsync()
{
    ...
    if (this->state != REC_STATE_STOPPED) this->stop();
...
{
    recordAsync();
    while (isRecording()) fiber_sleep(5);
    printChain();
}
...
bool StreamRecording::playAsync()
{
    ...
    if (this->downStream != NULL) this->downStream->pullRequest();
...
{
    if (this->lastUpstreamRate == DATASTREAM_SAMPLE_RATE_UNKNOWN) return this->upStream.getSampleRate();
