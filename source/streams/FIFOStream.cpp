...
ManagedBuffer FIFOStream::pull()
{
    if ((this->bufferLength > 0) && this->allowOutput) {
        ...
        for (int i = 0; i < FIFO_MAXIMUM_BUFFERS - 1; i++) buffer[i] = buffer[i + 1];
        ...
        if (this->bufferCount > 0 && downStream != NULL) downStream->pullRequest();
...
{
    DMESG("TapeDeck { bufferCount = %d/%d, bufferLength = %dB }", this->bufferCount, FIFO_MAXIMUM_BUFFERS,
          this->bufferLength);
}

int FIFOStream::pullRequest()
{
    if (this->bufferCount >= FIFO_MAXIMUM_BUFFERS) return DEVICE_NO_RESOURCES;
    ...
    if (bufferCount > 0 && this->allowOutput && downStream != NULL) return downStream->pullRequest();

    if (this->bufferCount >= FIFO_MAXIMUM_BUFFERS) return DEVICE_BUSY;
    return DEVICE_OK;
}
...
void FIFOStream::setOutputEnable(bool state)
{
    ...
    if (this->allowOutput == false && state) enabling = true;
    ...
    if (enabling && bufferCount > 0 && downStream != NULL) downStream->pullRequest();
}